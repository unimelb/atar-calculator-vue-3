  ...
  <div class="cell">
		<div v-bind:class="getCoursedTag(item.score)" class="btn-owner card card--news card--bdr">
			<a :href="getCoursedUrl(item.score)" class="card__inner">
                ...
                <h4 v-if="item.displayName" class="card__header">{{ item.displayName }}</h4>
				<h4 v-else class="card__header">{{ item.name }}<span aria-hidden="true">&nbsp;❯</span></h4>
                <h6 v-if="item.displaySubheading" class="card__header card__sub-heading">{{ item.displaySubheading }}<span aria-hidden="true">&nbsp;❯</span></h6>
			...
			<div class="card-links-section" style="display:none">
				...
				<ul class="card-links-list">
					<li class="card-links-link badge badge-mcs" >
						...
						<span v-else><span aria-hidden="true">&#10095; &nbsp;</span>{{friendly[index]}}: {{group.number}}</span>-->
                        <button-icon v-if="item.displayName && item.displaySubheading" :href="getCoursedUrl(item.score)">
                            <span class="link-heading">{{item.displayName}}</span>
                            <!-- <span class="link-subheading">{{item.displaySubheading}}</span> -->
                        </button-icon>
                        <!-- <p v-if="item.displayName && item.displaySubheading" class="link-subheading">{{item.displaySubheading}}</p> -->
						<button-icon v-else :href="getCoursedUrl(item.score)">{{item.name}}</button-icon>
