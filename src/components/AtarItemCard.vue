  ...
  <div class="cell">
		<div v-bind:class="getCoursedTag(item.score)" class="btn-owner card card--news card--bdr">
			<a :href="getCoursedUrl(item.score)" class="card__inner">
				...
				<h4 class="card__header">{{ item.name }}<span aria-hidden="true">&nbsp;❯</span></h4>
				...
				<p v-if="item.internationalGuarantee || item.domesticGuarantee" class="card__meta card-body-large" >
					...
					<span v-if="studentType == 'domestic'" class="text-italic secondary-atar"><br><strong>{{item.score.ug && item.score.ug.number}}</strong> Lowest selection rank {{item.year - 1}}</span>
...
<!--					<span v-if="studentType == 'international'" class="text-italic secondary-atar"><br><strong>{{item.score.international && item.score.international.number}}</strong> International minimum ATAR {{item.year}}</span>  -->
				...
				<p v-else-if="item.score.ug && item.score.international && item.score.ug.number && item.score.international.number" class="card__meta card-body-large">
					...
					<span v-if="studentType == 'domestic'" class="text-italic"><br>Lowest selection rank {{item.year - 1}}</span>
...
<!--					<span v-if="studentType == 'international' && item.score.international" class="text-italic"><br>International minimum ATAR {{item.year}}</span>						-->
			...
			<div v-if="item.score.indigenous || item.score.gdp" class="card-links-section">
				...
				<ul class="card-links-list">
					<li class="card-links-link badge badge-mcs" >
						<div v-if="item.score.indigenous">
							<span class="card__icon-info">
								<svg>
						...
						<!-- <svg aria-label="info" focusable="false" role="img">
    							<use xlink:href="#icon-info"></use>
    							<title>Info</title>
  							</svg> -->
						<!-- <span v-if="group.url">
							<span aria-hidden="true">&#10095;</span>
						<a :href="group.url" aria-hidden="false">{{friendly[index]}} <span class="aria-links">for {{item.name}}</span>: {{group.number}}</a></span>
						<span v-else><span aria-hidden="true">&#10095; &nbsp;</span>{{friendly[index]}}: {{group.number}}</span>-->
						<!-- <button-icon :href="getCoursedUrl(item.score)">{{item.name}}</button-icon>  -->
						<!-- <a href="#" aria-hidden="false">{{item.name}}</a>
						<span aria-hidden="true" class="card-links-link__icon">&#10095;</span> -->
					</li>
				</ul>
...
export default {
	data() {
		return {
			friendly: {
				...
				'indigenous': 'Degrees for Indigenous students',
				'other': 'Pathway (Guaranteed Pathways)'
			},
			localStudentType: this.studentType,  // Create a local property
			localScoreType: this.scoreType,  // Create a local property
		};
	},
	components: {
		ButtonIcon,
	...
	watch: {
		...
		scoreType: function(newVal, oldVal) {
			this.localScoreType = newVal;  // Watch the prop and update the local data property
		}
	},
	props: {
